...
export default function Bonus() {
  ...
  function load() {
    ...
    api
      .modules()
      .then((data) => setModules(data.modules.filter((mod) => mod.kind === 'bonus')))
  ...
  return (
    <div className="page">
      <h1>Bônus</h1>
      <p className="page-subtitle">Conteúdos extras liberados pela Andréa</p>
      {modules.length === 0 && <p>Nenhum bônus disponível ainda.</p>}
      {modules.map((mod) => (
          ...
          {mod.locked ? (
            <p className="module-locked-msg">
              {mod.lockReason === 'phase'
                ? 'Esse bônus ainda não foi liberado pra você. Continue evoluindo que a Andréa libera em breve!'
